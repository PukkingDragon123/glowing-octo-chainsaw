...
function createShowcase(ctx: ProductContext): ShowcaseItem {
  ...
  root.add(pickles);
  const fish = voxelMesh(soyFishVoxels(), { scale: 0.035, anchor: 'bottom-center' });
  ...
  const filling = voxelMesh(fillingVoxels(f), { scale: 0.06, anchor: 'bottom-center' });
  filling.position.set(0, APEX_Y - 0.15, 0);
  squash.add(filling);
  ...
  const faceTex = { sleep: faceArt('sleep').texture(), blink: faceArt('blink').texture(), open: faceArt('open').texture() };
  const faceMat = new THREE.MeshToonMaterial({ map: faceTex.sleep, gradientMap: toonGradient(), alphaTest: 0.5 });
  const face = new THREE.Mesh(new THREE.PlaneGeometry(44 / 48, 18 / 48), faceMat);
  face.position.set(0, FACE_TOP - 9 / 48, ONI.T / 2 + 0.002);
  squash.add(face);
...
function shelfModel(f: Flavor): THREE.Object3D {
  ...
  inner.add(tab);
  const filling = voxelMesh(fillingVoxels(f), { scale: 0.05, anchor: 'bottom-center' });
  filling.position.set(0, APEX_Y - 0.14, 0);
  inner.add(filling);
  return g;
}
